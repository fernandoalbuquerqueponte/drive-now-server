...
describe("Cars Route E2E Tests", () => {
  ...
  it("DELETE /api/cars/:carId should return 404 when car is not found", async () => {
    const { body: createdUser } = await request(app)
      .post("/api/users")
      .send({
        ...user,
    ...
    const response = await request(app)
      .delete(`/api/cars/${faker.string.uuid()}`)
      .set("Authorization", `Bearer ${createdUser.tokens.accessToken}`);
  ...
  it("PATCH /api/cars/:carId should return 401 when token is missing", async () => {
    const { body: createdUser } = await request(app)
      .post("/api/users")
      .send({
        ...user,
  ...
  it("POST /api/cars/reserve/:carId should return 401 when tokens is missing", async () => {
    const { body: createdUser } = await request(app)
      .post("/api/users")
      .send({
        ...user,
