...
describe("GetBookingsByCarIdUseCase", () => {
  ...
  it("should call GetBookingsRepository with correct params", async () => {
    const { sut, getBookingsRepository } = makeSut();
    const executeSpy = jest.spyOn(getBookingsRepository, "execute");
